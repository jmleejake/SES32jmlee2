...
public class HomeController {
	...
	public String newhome(HttpSession session) {
		...
		accList=dao.releaseList1(id, month);
		
		int income1=dao.checkIncome1(accList); // 이번 달 내역에 대한 고정 수입
		
		int income2=dao.checkIncome2(accList, income1); // 이번달 고정 수입 총 액수 - 고정지출 총 액수 = 가처분 소득
		
		int expense1= dao.checkExpense1(accList); // 이번달 변동 지출 총 액수
		
		int income3=dao.checkIncome3(id); // 사용자 회원 계정에 누적된 현재 비상금 액수
		
		int expense2 = dao.checkExpense2(id, beforeMonth); // 생활 적정 금액 산정
		
		int remainCheck=dao.remainCheckProcedure(id); // 총 잔여 금액
